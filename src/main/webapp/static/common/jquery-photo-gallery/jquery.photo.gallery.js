...
$.extend({
	//打开图片查看器
	openPhotoGallery : function(obj,classNameStr){
		if(!classNameStr){
			classNameStr = ".gallerys";
		}
		...
		var $gallerys = $(obj).closest(classNameStr),
			activeIndex=0,
			imgs = [];
		...
		$("<iframe></iframe").appendTo("body")
			.attr("id", "J_pg")
			.attr("src", "../../static/common/jquery-photo-gallery/gallery.html")
			.css({
				position : "absolute",
